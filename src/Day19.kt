data class Part(
	...
	companion object {
		fun fromString(str: String) = str.removeSurrounding("{", "}")
			.split(",")
			.associate { it.split("=").let { (char, int) -> char.single() to int.toInt() } }
			.let { Part(it) }
	}
}
...
data class InstructionLine(
	...
	fun run(part: Part): String = instructions.first { it.operation.invoke(part) }.destination


	companion object {
		fun fromString(str: String) =
			"""([a-zA-Z]+)\{(.+)\}""".toRegex().matchEntire(str)!!.groupValues.let { (_, name, instructionsStr) ->
				InstructionLine(
					name,
					instructionsStr.split(",").map { Instruction.fromString(it) }
				)
			}
	}
}
...
data class Instruction(
	...
	companion object {
		...
		fun fromString(str: String) =
			"""([a-z])([<>])(\d+):([a-zA-Z]+)""".toRegex().matchEntire(str)?.groupValues?.let {
				Instruction(it[4]) { part ->
					operationFromChar(it[2].single()).invoke(
						part.characteristics[it[1].single()]!!,
						it[3].toInt()
					)
				}
...
data class Instruction2(val destination: String, val operation: (Int) -> Boolean)
...
fun main() {
	...
	fun part1(input: List<String>) = input.indexOf("").let { splitIndex ->
		...
		Pair(
			instructionsStr.map { InstructionLine.fromString(it) },
			partsStr.map { Part.fromString(it) }
		)
		...
		while (partPositions.filter { it.key !in listOf("A", "R") }.any { it.value.second.isNotEmpty() }) {
			partPositions.filter { it.value.second.isNotEmpty() }
				.forEach { (lineName, value) ->
					if (value.first == null) return@forEach // do not run for A and R
					value.second.toList().forEach { part ->
						val destination = value.first!!.run(part)
	...
	fun part2(input: List<String>) = input.indexOf("").let { input.subList(0, it) }.associate { opLineStr ->
		"""([a-zA-Z]+)\{(.+)\}""".toRegex().matchEntire(opLineStr)!!.groupValues.let { (_, name, instructionsStr) ->
			Pair(
				name,
				instructionsStr.split(",").map { opStr ->
					"""([a-z])([<>])(\d+):([a-zA-Z]+)""".toRegex()
						.matchEntire(opStr)?.groupValues?.let { (_, char, op, value, dest) ->
							Operation(char.single(), op.single(), value.toInt(), dest)
						} ?: (Operation('a', 'a', 0, opStr))
				}
			)
		}
		...
		rangesPositions["A"]!!.sumOf {
			it.values.map { it.count() }.fold(1.toLong()) { acc, size -> acc * size.toLong() }.toLong()
		}
	}
